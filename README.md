...
2. Start the development server (runs both the React frontend and Express backend concurrently):
   ```bash
   npm run dev
   ```

3. The application will be available on your designated local port (typically `http://localhost:3000`).

### Starting as Admin
To access the educator dashboards, use the following default credentials:
...
npm run start
```
This optimizes the frontend assets and securely deploys the Express server for production use.
